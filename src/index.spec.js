...
async function mockContext(options) {
    const { branch, prTitle = 'title', prBody = 'body', jiraAccount = 'account', ticketRegex = '^A1C-\\d+', ticketRegexFlags = 'i', exceptionRegex = '^dependabot\\/', cleanTitleRegex = '^\\s*A1\\s+c\\s+\\d+\\s*', preview = 'preview', updateStatus = HTTP_STATUS_SUCCESS, contextRepoObject = {
        owner: DEFAULT_REQUEST_OPTIONS.owner,
        repo: DEFAULT_REQUEST_OPTIONS.repo,
    }, hasPullRequestContext = true, } = options;
    ...
    jest.doMock('@actions/core', () => ({
        getInput: jest.fn((input) => {
            if (input === 'github-token')
                return 'abc123';
            if (input === 'jira-account')
                return jiraAccount;
            if (input === 'ticket-regex')
                return ticketRegex;
            if (input === 'ticket-regex-flags')
                return ticketRegexFlags;
            if (input === 'exception-regex')
                return exceptionRegex;
            if (input === 'clean-title-regex')
                return cleanTitleRegex;
...
describe('#pull-request', () => {
    ...
    describe('when current branch includes Jira ticket', () => {
        ...
        describe('and when PR update request fails', () => {
            ...
            it('sets error status', () => {
                expect(errorSpy).toHaveBeenCalledWith(`Updating the pull request has failed with ${HTTP_STATUS_ERROR}`);
            });
        });
    });
    ...
    describe('when current branch does not include Jira ticket', () => {
        describe('and when exception-regex input does not match current branch', () => {
            ...
            it('sets failed status', () => {
                expect(setFailedSpy).toHaveBeenCalledWith(expect.stringContaining('Neither current branch nor title start with a Jira ticket'));
            });
    ...
    describe('when clean-title-regex input is provided', () => {
        ...
        it('cleans PR title', () => {
            expect(prUpdateSpy).toHaveBeenCalledWith(expect.objectContaining({ title: `${ticket} - feature` }));
        });
    });
    describe('when clean-title-regex input is missing', () => {
        ...
        it('does not clean PR title', () => {
            expect(prUpdateSpy).toHaveBeenCalledWith(expect.objectContaining({ title: `${ticket} - ${prTitle}` }));
        });
    });
